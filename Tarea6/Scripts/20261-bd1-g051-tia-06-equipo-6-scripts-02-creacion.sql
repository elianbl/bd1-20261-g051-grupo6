...
CREATE TABLE colmena(
	...
	estado_abeja VARCHAR(50) NOT NULL CHECK(estado_abeja IN ('activa', 'enferma', 'inactiva')),
...
CREATE TABLE apicultor_producto(
    ...
    CONSTRAINT pk_apicultor_producto PRIMARY KEY (id_usuario, id_producto),
    CONSTRAINT fk_apicultor_prod_usuario FOREIGN KEY (id_usuario) REFERENCES usuarios(id_usuario),
    CONSTRAINT fk_apicultor_prod_producto FOREIGN KEY (id_producto) REFERENCES producto(id_producto)
...
CREATE TABLE publicacion(
	...
	CONSTRAINT fk_publicacion_mercado FOREIGN KEY (id_mercado) REFERENCES mercado(id_mercado)
...
CREATE TABLE pedido(
	...
	CONSTRAINT fk_pedido_mercado FOREIGN KEY (id_mercado) REFERENCES mercado(id_mercado)
...
CREATE TABLE documento(
    ...
    CONSTRAINT chk_documento_exclusivo CHECK (

    (id_usuario IS NOT NULL AND id_apiario IS NULL AND id_lote IS NULL) OR

    (id_usuario IS NULL AND id_apiario IS NOT NULL AND id_lote IS NULL) OR
    
    (id_usuario IS NULL AND id_apiario IS NULL AND id_lote IS NOT NULL)
)
